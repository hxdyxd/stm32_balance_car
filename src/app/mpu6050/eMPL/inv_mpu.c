...
#ifdef MOTION_DRIVER_TARGET_STM32
//hal
...
#define delay_ms  delay_ms_mpu

void get_ms_mpu(unsigned long *count)
{
    *count = (unsigned long)hal_read_TickCounter();
}

void delay_ms_mpu(unsigned long num_ms)
{
    uint64_t last = hal_read_TickCounter();
    while( hal_read_TickCounter() - last < num_ms);
}
...
#define log_e    DebugOutStr
#define log_i    DebugOutStr
